...
function getSorter(order = 1, pathOrComparator = null) {
    ...
    if (typeof pathOrComparator === 'function') {
        ...
        sorter = (a, b) => {
            const aValue = Utils.getValueAtPath(a, pathOrComparator);
            const bValue = Utils.getValueAtPath(b, pathOrComparator);
...
class ArrayHandler extends GenericHandler {
    ...
    static unique(arr, pathOrComparator = null) {
        ...
        for (let y = 0; y < arr.length - 1; y++) {
            const a = path
                ? Utils.getValueAtPath(arr[y], path)
                : arr[y];

            for (let z = y + 1; z < arr.length; z++) {
                const b = path
                    ? Utils.getValueAtPath(arr[z], path)
                    : arr[z];
    ...
    static group(arr, path) {
        ...
        for (const value of arr) {
            const mapKey = path ? Utils.getValueAtPath(value, path) : value;
    ...
    static removeDuplicates(arr, pathOrComparator = null) {
        ...
        for (let y = 0; y < filteredArr.length - 1; y++) {
            const a = path
                ? Utils.getValueAtPath(filteredArr[y], path)
                : filteredArr[y];

            for (let z = y + 1; z < filteredArr.length; z++) {
                const b = path
                    ? Utils.getValueAtPath(filteredArr[z], path)
                    : filteredArr[z];
