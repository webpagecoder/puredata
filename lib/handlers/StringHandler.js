...
function splitOnDelims(str, chars) {
    const split = str.length > 0
        ? str.split(RegexCache('[' + escapeForRegex(chars) + ']+'))
        : [];
    const final = [];
...
function validateWithCheckDigit(str, {
    weights = [2, 1],
    mod = 10,
    transform = x => x,
    reverse = false
} = {}) {
    return str.length > 0 && generateCheckDigit(str.slice(0, -1), {
        weights,
        mod,
        transform,
        reverse
...
function regexMatch(str, regex, options = {}) {
    const {
        allowedDelims,
        delim,
        allowLooseFormat
    ...
    let matchData;
    const bareStr = replaceChars(str, allowedDelims + delim);
    ...
    if (allowLooseFormat) {
        ...
        matchData = Array.isArray(regex)
            ? RegexCache('^(' + regex.join(')' + escapeForRegex(delim) + '(') + ')$')
                .exec(str)
            : regex.exec(str);
    }
...
function replaceChars(str, delims, replacement = '') {
    return str.replace(RegexCache('[' + escapeForRegex(delims) + ']+', 'g'), replacement);
}
...
class StringHandler extends GenericHandler {
    ...
    static e164(str, options = StringHandler.matchingDefaults) {
        ...
        const matchData = regexMatch(
            str,
            new RegExp('^\\+[1-9](?:' + escapeForRegex(delim) + '?\\d){6,14}$'),
            {
                delim,
                allowedDelims,
                allowLooseFormat
    ...
    static gtin(str, options = StringHandler.matchingDefaults) {
        ...
        for (const regex of patterns) {
            ...
            if (matchData) {
                const [bareStr, ...parts] = matchData;
                if (!validateWithCheckDigit(bareStr, { weights: [3, 1], reverse: true })) {
    ...
    static luhn(str) {
        return validateWithCheckDigit(str, {
            weights: [2, 1],
            mod: 10,
    ...
    static maxWords(str, max, allowedDelims = StringHandler.matchingDefaults.allowedDelims) {
        const count = splitOnDelims(str, allowedDelims).length;
        return count <= max
            ? pass(str)
    ...
    static numeric(str, options = {}) {
        ...
        const parts = RegexCache(
            '^(\\+?)(-?)'
            + looseSpacing
            + `(?:${leadingSymbolArr.map(escapeForRegex).join('|')})`
            + looseSpacing
            + '(.+?)'
            + looseSpacing
            + `(?:${trailingSymbolArr.map(escapeForRegex).join('|')})`
            + looseSpacing
            + '(\\+?)(-?)$'
            , ignoreCase ? 'i' : '')
            .exec(str);
        ...
        let integralRegex = thousandsDelim
            ? `^(|0|[1-9]\\d{0,2}(?:${escapeForRegex(thousandsDelim)}\\d{3})*)$`
            : '^(\\d*)$';
        ...
        const integralNumPlain = integral.replace(new RegExp(escapeForRegex(thousandsDelim), 'g'), '');
    ...
    static path(str, options = StringHandler.matchingDefaults) {
        ...
        const fullRegex = [
            ...
            (
                ...
                '\\.(' + fileExtensionsArray.map(ext => escapeForRegex(ext)).join('|') + ')'
            ),
            '))\\1',
            '$',
        ].join('');
    ...
    static repetition(str, fragment, options = StringHandler.matchingDefaults) {
        ...
        if (!otherText) {
            const fullRegex = `^(?=((${escapeForRegex(fragment)}){${min},${max === null ? '' : max}}))\\1$`;
        ...
        const difference =
            str.length -
            str.replace(RegexCache(escapeForRegex(fragment), 'g' + (ignoreCase ? 'i' : '')), '').length;
    ...
    static url(str, options = StringHandler.matchingDefaults) {
        ...
        const portValueNum = parseNumber(portValue);
    ...
    static collapseRepeats(str, char) {
        return pass(str.replace(RegexCache('(' + (char ? escapeForRegex(char) : '.') + ')\\1+', 'g'), '$1'));
    }
    ...
    static toCamelCase(str, allowedDelims = StringHandler.matchingDefaults.allowedDelims) {
        return this.toDelimited(str, {
            ...
            transformer2: word => word[0].toUpperCase() + word.slice(1).toLowerCase()
        });
    }
    ...
    static toDelimited(str, options = {}) {
        ...
        return pass(
            splitOnDelims(str, allowedDelims)
    ...
    static toPascalCase(str, allowedDelims = StringHandler.matchingDefaults.allowedDelims) {
        return this.toDelimited(str, {
            allowedDelims,
            delim: '',
            transformer: word => word[0].toUpperCase() + word.slice(1).toLowerCase()
        });
    }
    ...
    static toSentenceCase(str, allowedDelims = StringHandler.matchingDefaults.allowedDelims) {
        return this.toDelimited(str, {
            allowedDelims,
            delim: ' ',
            transformer1: word => word[0].toUpperCase() + word.slice(1).toLowerCase(),
            transformer2: word => word.toLowerCase()
        });
    }
    ...
    static trim(str, chars) {
        const finalChars = chars === ' ' ? '\\s' : escapeForRegex(chars);
