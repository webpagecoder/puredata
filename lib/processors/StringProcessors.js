...
const NumberProcessors = require('./NumberProcessors.js');
...
class StringProcessors extends GenericProcessors {
    ...
    static creditCard(str, options = {}) {
        ...
        for (const [, regex, checkLuhn] of cards) {
            ...
            if (matchData) {
                ...
                if (checkLuhn && !self.luhn(bareStr).pass) {
    ...
    static email(str, options = StringProcessors.matchingDefaults) {
        ...
        if (parts.length !== 2 || self.domain(parts[1]).fail) {
    ...
    static ipCidr(str) {
        return self.ipCidrV4(str).pass || self.ipCidrV6(str).pass
            ? pass(str)
    ...
    static ipCidrV4(str) {
        ...
        return num !== null && self.ipV4(parts[0]).pass && NumberProcessors.between(num, 0, 32).pass
            ? pass(str)
    ...
    static ipCidrV6(str) {
        ...
        const num = NumberUtils.toNumber(parts[1]);
        return num !== null && self.ipV6(parts[0]).pass && NumberProcessors.between(num, 0, 128).pass
            ? pass(str)
    ...
    static label(str, options = StringProcessors.matchingDefaults) {
        ...
        if (
            !self.lengthBetween(str, 1, 63).pass
            || str.startsWith('-')
            || str.endsWith('-')
        ) {
    ...
    static measurement(str, options = {}) {
        ...
        const result = self.numeric(str, mergedOptions);
        return result.pass
            ? pass(result.value)
    ...
    static url(str, options = StringProcessors.matchingDefaults) {
        ...
        const
            ...
            isIp = self.ipV4(hostValue).pass || self.ipV6(ipv6Value).pass,
            isDomain = self.domain(hostValue).pass,
            isLabel = self.label(hostValue).pass,
    ...
    static meetsComplexity(str, options = {}) {
        const {
            minLen = 8,
            maxLen = 100,
            minLowercase = 1,
...
// Export the class
const self = module.exports = StringProcessors;