...
class StringUtils {
    ...
    static trimLeft(str, chars = ' ') {
        const finalChars = chars === ' ' ? '\\s' : StringUtils.escapeForRegex(chars);
    ...
    static collapseRepeats(str, char = null) {
        ...
        return str.replace(RegexCache('(' + StringUtils.escapeForRegex(char) + ')\\1+', 'g'), '$1');
    }
    ...
    static getMatchData(str, regex, options = {}) {
        ...
        let matchData;
        const bareStr = StringUtils.replaceDelims(str, allowedDelims + delim);
    ...
    static replaceDelims(str, delims, replacement = '') {
        return str.replace(RegexCache('[' + StringUtils.escapeForRegex(delims) + ']+', 'g'), replacement);
    }

    static splitOnDelims(str, chars) {
        const trimmed = StringUtils.trim(str, chars);
        return trimmed.length > 0
            ? trimmed.split(RegexCache('[' + StringUtils.escapeForRegex(chars) + ']+'))
            : [];
    }
    ...
    static validateWithCheckDigit(str, {
        ...
        return str.length > 0 && StringUtils.generateCheckDigit(str.slice(0, -1), {
            weights,
            mod,
            transform,
            reverse
    ...
    static validateLuhn(str) {
        return StringUtils.validateWithCheckDigit(str, {
            weights: [2, 1],
            mod: 10,
