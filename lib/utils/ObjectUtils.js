...
const Path = require("../path/PathFactory");
...
class ObjectUtils {
    ...
    static deepMerge(parent, child) {
        ...
        while (stack.length) {
            ...
            for (const key of Object.keys(child)) {
                if (self.isPlainObject(parent[key]) && self.isPlainObject(child[key])) {
                    stack.push([
                        parent[key],
                        child[key]
                    ]);
    ...
    static removePath(obj, path) {
        const [pointer, key] = self.getPointerAtPath(obj, path, { create: false });
        if (pointer) {
            delete pointer[key];
            return true;
        }
    ...
    static getAllPathsCount(obj, options = {}) {
        let count = 0;
        for (const _ of self.getAllPaths(obj, '', options)) {
            count++;
        }
        return count;
    }
    ...
    static getDepthAndKeyCount(obj, {
        ...
        for (const key of keys) {
            ...
            if (self.isObject(value)) {
                const childResult = self.getDepthAndKeyCount(value, {
                    depth,
                    keyCount,
                    maxDepth,
                    maxKeyCount,
    ...
    static hasPath(obj, path) {
        const [pointer,] = self.getPointerAtPath(obj, path, { create: false, overwrite: false });
    ...
    static unflatten(x, { separator = '/' } = {}) {
        ...
        for (const path of Object.keys(x)) {
            ...
            for (let i = 0; i < max; i++) {
                if (!self.isPlainObject(pointer[pieces[i]])) {
                    pointer[pieces[i]] = {};
                }
...
const self = module.exports = ObjectUtils;