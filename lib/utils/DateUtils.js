...
import DATE_TYPES from './DateTypes.js';
...
class DateUtils {
    ...
    static parseFromHuman(dateString, {
        ...
        if (!this.areDayAndDateValid({ YYYY, MM, DD })) {
            return null;
        }
    ...
    static parseFromIsoOrdinal(dateString) {
        ...
        if (+DDD === 366 && !this.isLeapYear(YYYY)) {
            return null;
        }
    ...
    static parseFromIsoWeek(dateString) {
        ...
        if (+ww === 53 && !this.has53IsoWeeks(YYYY)) {
            return null;
        }
    ...
    static toIsoOrdinal(date) {
        ...
        return `${date.getUTCFullYear()}-${Utils.padLeft(dayOfYear.toString(), 3, '0')}`;
    }
    ...
    static toIsoWeek(date) {
        ...
        return `${target.getUTCFullYear()}-W${Utils.padLeft(weekNum.toString(), 2, '0')}-${dayNum}`;
    }
