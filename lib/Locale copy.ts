...
class Locale {
    ...
    public static registry: Record<string, TextTree> = {};

    protected _languageKey?: string;
    protected _overrides: TextTree = {};
    protected _text: Locale | TextTree = {};

    static register(languageKey: string, text: TextTree = {}): void {
    ...
    translate(path: Path, allowMultiple: boolean = false): string | string[] | TextTree {
        ...
        if (pathPointer == null) {
            throw new Error('Nonexistent path in language file: ' + path);
        }
        ...
        throw new Error('Invalid path for translation: ' + path);
    }
