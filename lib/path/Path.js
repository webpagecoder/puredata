...
class Path {
    ...
    constructor(string = '') {
        ...
        const normalized = (this.isAbsolute ? separator : '')
            + (up + separator).repeat(this.upCount)
            + this._keys.join(separator);
        this.string = normalized || (this.isAbsolute ? separator : self);
        this.isSelf = normalized === self;
    }
    ...
    shiftKeys(count = 1) {
        ...
        const { separator } = this;
        return Path.create(separator + this._keys.slice(count).join(separator));
    }
