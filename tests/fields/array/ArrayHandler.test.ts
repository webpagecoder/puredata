...
describe('ArrayHandler mutators', () => {
	...
	it('padEnd', () => {
		runCases(
			(input: unknown[], options?: { targetLength: number; padValue?: unknown }): ValidationResult =>
				handler.padEnd(input, options?.targetLength ?? 0, options?.padValue),
			[
				{ input: [1], options: { targetLength: 3, padValue: 0 }, pass: true, value: [1, 0, 0] },
				{ input: [1], options: { targetLength: 3 }, pass: true, value: [1, null, null] },
				{ input: [1, 2, 3], options: { targetLength: 2, padValue: 0 }, pass: true, value: [1, 2, 3] }
			]
		);
	});
	...
	it('splice', () => {
		runCases(
			(input: unknown[], options?: { startIndex: number; deleteCount?: number; insertValues?: unknown[] }): ValidationResult =>
